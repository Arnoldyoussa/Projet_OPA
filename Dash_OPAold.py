...
@app.callback(Output('page-content', 'children'),  # Sortie : contenu de l'élément 'page-content'
              [Input('url', 'pathname')])  # Entrée : valeur de l'élément 'url' (pathname)
def display_page(pathname):
    # Vérifie si l'URL actuelle correspond à '/Backtest'
    if pathname == '/Backtest':
        # Si l'URL contient '/Backtest', affiche la page de backtest
...
def on_button_click(n_clicks, date_debut, date_fin, capital, currency_pair): 
    if n_clicks: 
        ...
        output = Bot.Get_Backtest(currency_pair, date_debut, date_fin, capital)
...
def update_graph_containerv2(selected_method, output): 
    ...
    for methode, resultats in output.items():
        if methode == selected_method:
            ...
            return fig_prediction, fig_simulation, Table_Rapport
